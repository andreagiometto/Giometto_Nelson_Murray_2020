...
% coefficient for yeast cells, constant toxin production rate and 3D toxin
...
function [] = spatial_model_logistic_3D_const_toxin_const_diffusion()
...
%     a2=2.36e-09; % mL/(cells h^2) (toxin production coefficient)
    a2=9.60e-10;
...
%     transfers=1+13; % number of transfers
    ...
    for t=1:T
        ...
        for j=2:(N-1) % top - bottom boundary
            ...
            % growth independent toxin production rate
            c1(M,j)=(c1(M-1,j)+a1*dz/Dt*n1(j,1))/(1+b*dz/Dt*(n1(j,1)+n2(j,1)));
            c2(1,j)=c2(2,j); % bottom
            % growth independent toxin production rate
            c2(M,j)=(c2(M-1,j)+a2*dz/Dt*n2(j,1))/(1+b*dz/Dt*(n1(j,1)+n2(j,1)));
        end
        ...
        for j=2:N-1 % n1, n2 update
            n1(j,1)=n1T(j,1)+dt*g*n1T(j,1)*(1-(n1T(j,1)+n2T(j,1))/K)+dt*Dfactor*Dy*(1/r(j)*(n1T(j+1,1)-n1T(j-1,1))/(2*dr)+(n1T(j+1,1)-2*n1T(j,1)+n1T(j-1,1))/(dr^2))-dt*n1T(j,1)*c2T(M,j);
            n2(j,1)=n2T(j,1)+dt*g*n2T(j,1)*(1-(n1T(j,1)+n2T(j,1))/K)+dt*Dfactor*Dy*(1/r(j)*(n2T(j+1,1)-n2T(j-1,1))/(2*dr)+(n2T(j+1,1)-2*n2T(j,1)+n2T(j-1,1))/(dr^2))-dt*n2T(j,1)*c1T(M,j);
        end
        n1(1,1)=n1(2,1);
        n1(N,1)=n1(N-1,1);
        n1T=n1;
